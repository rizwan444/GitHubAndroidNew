...
public class DataBase extends SQLiteOpenHelper {
    ...
    public List<String> GetAPPTDETAILS(String Patid){
        SQLiteDatabase db = this.getReadableDatabase();
        List<String> labels = new ArrayList<String>();
        String query="SELECT * FROM "+ApptTABLE_NAME+" WHERE "+ApptCOL_4+" = '"+Patid+"' ";
        Cursor cursor= db.rawQuery(query,null);
        if (cursor.moveToFirst()) {

            do {
                labels.add("DOCNAME: "+cursor.getString(2));
                labels.add("TIME: "+cursor.getString(3));
                labels.add("DATE "+cursor.getString(4));
    ...
    public List<String> GetDocDetails(String docid){
        SQLiteDatabase db = this.getReadableDatabase();
        List<String> labels = new ArrayList<String>();
        String query="SELECT * FROM "+DTABLE_NAME+" WHERE "+DCOL_1+" = '"+docid+"' ";
        Cursor cursor= db.rawQuery(query,null);
        if (cursor.moveToFirst()) {

            do {
                labels.add("NAME:\t"+cursor.getString(1));
                labels.add("SPECIALIZATION: "+cursor.getString(4));
                labels.add("Average Appointment: "+cursor.getString(5));
                labels.add("FEE: "+cursor.getString(6));
                labels.add("Clinic: "+cursor.getString(8)+" "+cursor.getString(9));
                labels.add("Contact: "+cursor.getString(10));
    ...
    public List<String> FetchDoctors(){
        ...
        if (cursor.moveToFirst()) {
            ...
            previous=cursor.getString(1);
            do {
                if(previous.matches(cursor.getString(1))){
    ...
    public Boolean getPatEmails(String email){
        SQLiteDatabase db= this.getReadableDatabase();
        Cursor cursor= db.rawQuery("SELECT * FROM "+PTABLE_NAME+" WHERE "+PCOL_4+"='"+email+"' ",null);
        Boolean res=false;
